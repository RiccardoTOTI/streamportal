...
import aiohttp
import asyncio
from app.utils import check_url_exists, check_url_exists_async
...
async def search_movies(text_search, option_language, headers):
    """Quick search that returns basic movie information without checking streaming availability."""
    movies_list = []
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for page in range(1, 6):
            url = f"https://api.themoviedb.org/3/search/movie?query={text_search}&include_adult=false&language={option_language}&page={page}"
        ...
        for page_num, page_data in enumerate(page_results, 1):
            ...
            if isinstance(page_data, dict) and page_data.get("results"):
                logger.debug(
                    f"Processing page {page_num} with {len(page_data['results'])} movies",
                    extra_fields={"page": page_num, "movies_count": len(page_data["results"])}
                )
...
async def get_movie_details(movie_id, option_language, headers):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        movie_url = f"https://api.themoviedb.org/3/movie/{movie_id}?language={option_language}"
        
        try:
            async with session.get(movie_url, headers=headers) as response:
                if response.status == 404:
                    raise NotFoundError(f"Movie with ID {movie_id} not found", "Movie", movie_id)
            ...
            logger.error(
                f"Network error fetching movie details",
            ...
            if isinstance(e, (NotFoundError, ExternalAPIError)):
                raise
            logger.error(
                f"Unexpected error fetching movie details",
                extra_fields={"movie_id": movie_id, "error": str(e)}
            )
            raise ExternalAPIError(f"Failed to fetch movie details: {str(e)}", "TMDB API")
