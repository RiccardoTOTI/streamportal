...
COPY --chown=python_application:python_application env.example ${PYTHONPATH}/.env.example
...
CMD ["sh", "-c", "uvicorn --workers 1 --host 0.0.0.0 --port $APPLICATION_SERVER_PORT app.main:app"]
